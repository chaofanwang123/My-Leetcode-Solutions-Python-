class Solution:
    def findAllConcatenatedWordsInADict(self, words):
        ...
        :rtype: List[str]
        """
        def dfs(s,idx,words,minlen,maxlen,n,List):
            if s=='':
            ...
            if m>=minlen:
                for i in range(n):
                    if i!=idx and len(words[i])<len(words[idx]):
                        index=s.find(words[i])
                        if index!=-1:
                            if dfs(s[:index]+s[index+len(words[i]):],idx,words,minlen,maxlen,n,List):
                                return True
            return False
        n=len(words)
        if n<=2:
        ...
        List=[]
        i=n-1
        while i>0 and len(words[i])>=2*minlen:
            dfs(words[i],i,words,minlen,maxlen,n,List) 
            i-=1
        return List
